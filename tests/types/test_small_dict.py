...
class TestSmallDictZeoConflicts:
    sheraf_database_extra_kwargs = {"nestable": True}

    def test_empty_dict_no_conflict(self, sheraf_zeo_database):
    ...
    def test_same_item_same_modification_no_conflict(self, sheraf_zeo_database):
        with sheraf.connection(commit=True) as conn:
    ...
    def test_different_item_modification_no_conflict(self, sheraf_zeo_database):
        with sheraf.connection(commit=True) as conn:
            conn.root()["mydict"] = sheraf.types.SmallDict(
    ...
    def test_same_item_different_modification_conflict(self, sheraf_zeo_database):
        with sheraf.connection(commit=True) as conn:
            conn.root()["mydict"] = sheraf.types.SmallDict({"something": None})
